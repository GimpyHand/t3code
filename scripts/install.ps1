# Installs the And I Code CLI from a GitHub Release archive on Windows. Needs
...
if ($interactive) {
  ...
  for ($i = 0; $i -lt $mark.Length; $i++) {
    ...
    $label = if ($i -eq 1) { "     ${bold}And I Code$reset" } elseif ($i -eq 2) { "     ${muted}CLI installer$reset" } else { "" }
...
if ((Test-Path $marker) -and ((Get-Content $marker -Raw).Trim() -eq $version)) {
  ...
  try {
    ...
    [Console]::Error.WriteLine("  ${muted}Installing$reset And I Code $bold$version$reset`n")
    Step "Downloading..."
    try {
...
if ($interactive) { [Console]::Error.Write("`r$esc[2K") }
[Console]::Error.WriteLine("  ${green}Installed And I Code $version$reset`n")
