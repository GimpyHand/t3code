...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: "Display name for the script, shown in the And I Code scripts menu.",
  }),
  command: trimmedNonEmpty({
    description: "Shell command executed in a And I Code terminal at the project root.",
  }),
  ...
  description: "A project script that team members can import into And I Code.",
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          'Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before And I Code\'s built-in icon locations.',
      },
  ...
  defaultThreadEnvMode: Schema.optionalKey(
    ThreadEnvMode.annotate({
      description:
        'Where new threads start for this repository: "worktree" for a fresh git worktree, "local" for the current checkout. A per-project setting in And I Code overrides this; when neither is set, the global default applies.',
    }),
  ),
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description: "Project scripts shared with everyone who opens this repository in And I Code.",
      })
  ...
  description:
    "Checked-in project configuration for And I Code (t3.json at the repository root). See https://t3.codes for documentation.",
});
