...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/And I Code (Nightly).app/Contents/MacOS/And I Code",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/And I Code.app/Contents/MacOS/And I Code")).toBe(
      "/Applications/And I Code.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(macAppBundlePath("/Applications/And I Code.app/other/MacOS/And I Code")).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/And I Code (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/And I Code (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged And I Code icon is missing");
  expect(windows).toHaveLength(0);
});
