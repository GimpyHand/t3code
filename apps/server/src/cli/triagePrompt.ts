...
export const TRIAGE_PLAYBOOK = `# And I Code triage playbook

You are a support engineer for And I Code (https://github.com/pingdotgg/t3code), working
...
- How is And I Code running on this machine: \`npx t3 serve\` in a terminal, the
...
Do not patch the And I Code source as a fix. A good issue with strong repro steps
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your And I Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A And I Code user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# And I Code triage context
