...
const INITIAL_THREAD_TITLE_PROMPT = `Generate a title that will help the user recognize this And I Code thread weeks later.
...
function regenerateThreadTitlePrompt(previousTitle: string): string {
  return `Regenerate the title for an existing And I Code thread so the user can recognize it weeks later.
